...
- `DEV_API_KEY` - API key generated by the `generate_keys.sh` script. This is used by the demo frontend to automatically authenticate requests.
- `ALLOW_DEMO_FRONTEND` - Possible values are `allow|null` Allow access to the demo frontend. Should be disabled on production systems.
- `ALLOW_CRD` - Possible values are `allow|null`. Flag to toggle if ticket sales should be posted to CRD backend. Should be disabled on local development environments.
